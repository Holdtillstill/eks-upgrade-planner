...
function DesignNav({ active, setRoute }: { active: DesignRoute; setRoute: (route: AppRoute) => void }) {
  return <nav className="design-nav" aria-label="Design variants">
    <a className="brand" href="/1" onClick={(event) => { event.preventDefault(); navigate('/1', setRoute); }}><span/>EKS Upgrade Planner</a>
    <div>{routes.map((r) => <button
      type="button"
      key={r.path}
      className={active === r.path ? 'active' : ''}
      aria-label={`Open ${r.name} design exploration`}
      onClick={() => navigate(r.path, setRoute)}
    >{r.path}<em>{r.name}</em></button>)}</div>
  </nav>;
}
...
function DesignFour() {
  ...
  return <main className="page d4">
    <section className="os-frame">
      <aside className="os-rail"><strong>Signal OS</strong><button type="button" aria-label="Open command palette">⌘K</button><button type="button">API</button><button type="button">EOL</button><button type="button">Cost</button></aside>
...
export default function DesignExplorations({ route, setRoute }: { route: DesignRoute; setRoute: (route: AppRoute) => void }) {
  return <>
